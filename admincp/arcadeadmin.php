...
if ($_REQUEST['do'] == 'checkgames')
{
	...
	require_once(DIR . '/includes/class_xml1.php');
	...
	if (!is_array($link['game']))
	{
		...
		if ($thisgame['doinstall']==1)
		{
			...
			if (function_exists('curl_init'))
           {
            ...
            curl_setopt ($ch, CURLOPT_URL, 'http://www.v3arcade.com/link/init.php?do=download&licenseid=' . $vbulletin->GPC['licenseid']);
            curl_setopt ($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt ($ch, CURLOPT_CONNECTTIMEOUT, $timeout);
            $contents = curl_exec($ch);
            curl_close($ch);
           }
           // If we cant use cURL lets use fopen..
           else
           {
			...
			if ($gametar->openTAR($contents, 1))
			{
				foreach ($gametar->files as $key => $val)
				{
					if (!strpos($val['name'], '/'))
					{
						...
						{
					                            case '.gif':
                            $newfile = $_SERVER['DOCUMENT_ROOT'].'/images/arcade/' . $val['name'];
                            break;
                            case '.swf':
                            $newfile = $_SERVER['DOCUMENT_ROOT'].'/games/' . $val['name'];
                            break;
						}
