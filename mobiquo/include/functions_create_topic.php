...
function mobiquo_build_new_post($type = 'thread', $foruminfo, $threadinfo, $postinfo, &$post, &$errors)
{
	...
	{
		if(file_exists(DIR . '/includes/class_humanverify.php'.SUFFIX)){
			require_once(DIR . '/includes/class_humanverify.php'.SUFFIX);
		} else {
	...
	if ($type == 'thread' AND !$dataman->fetch_field('prefixid') AND ($foruminfo['options'] & $vbulletin->bf_misc_forumoptions['prefixrequired']))
	{
		...
		if(file_exists(DIR . '/includes/functions_prefix.php'.SUFFIX)){
			require_once(DIR . '/includes/functions_prefix.php'.SUFFIX);
		} else {
